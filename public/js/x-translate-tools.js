...
(() => {
  ...
  const DEFAULT_PROMPT = '你是专业论坛翻译助手。请把用户提供的内容从 {{sourceLang}} 翻译为 {{targetLang}}。采用自然直译，保留原有语气、换行、链接、Markdown、代码块、用户名、表情和列表结构。只输出译文，不要解释。';
  const LANGS = [
  ...
  function xEnsureCategoryToolbar(options = {}) {
    ...
    let toolbar = $('#haa9-category-toolbar');
    if (!toolbar) {
      toolbar = document.createElement('div');
      toolbar.id = 'haa9-category-toolbar';
      toolbar.className = 'haa9-category-toolbar';
      toolbar.innerHTML = `<button type="button" class="x-ai-settings-btn haa9-category-settings-btn"><i class="fa-solid fa-sliders"></i><span>AI翻译</span></button><button type="button" class="haa9-category-compose-btn"><i class="fa-solid fa-pen"></i><span>发帖</span></button>`;
      const anchor = $('[component="category/toolbar"], .category-tools, .sticky-tools, [component="category/header"], .category-header, .category') || document.body.firstElementChild || document.body;
      if (anchor && anchor.parentNode) anchor.parentNode.insertBefore(toolbar, anchor.nextSibling);
      else document.body.prepend(toolbar);
      $('.haa9-category-settings-btn', toolbar).addEventListener('click', event => { event.preventDefault(); xOpenTranslateSettings(); });
      $('.haa9-category-compose-btn', toolbar).addEventListener('click', event => {
        event.preventDefault();
        const fab = $('#haa9-fab');
        if (fab && typeof options.openComposer === 'function') options.openComposer();
        else if (fab) fab.click();
        else clickNativeComposer();
      });
    }
    xBindNativeTitleTranslation(document);
  }

  function xRemoveCategoryToolbar() { $('#haa9-category-toolbar')?.remove(); }
