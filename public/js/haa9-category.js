/* =========================================================
   HAA9 unified v2 launch
   NodeBB 4.x / category cid 6 list mode, all topic detail pages

   Replace old stacked HAA8/HAA9 JS with this single file.
...
(() => {
  ...
  const CONFIG = {
    cid: 6,
    // List/category feed enhancement remains scoped to cid 6.
    // Topic-detail media hydration is enabled for all categories by default.
  ...
  try {
    const storedPluginConfig = JSON.parse(localStorage.getItem('haa9-plugin-config') || '{}');
    if (storedPluginConfig && storedPluginConfig.client) Object.assign(CONFIG, storedPluginConfig.client);
  } catch (_) {}
  ...
  function isTargetTopicPage() {
    ...
    // Topic detail pages should use the enhanced media/detail rendering across all boards.
    // Keep CONFIG.cid for category/topic-list pages only, so list cards do not suddenly
    // take over every board.
    if (CONFIG.topicDetailAllCids === true) return true;
  ...
  function getCounts(li) {
    const commentsNode = $('[component="topic/post-count"], .stats-postcount .fs-5', li);
    const likesNode = $('.stats-votes .fs-5, [component="topic/vote-count"], [component="post/vote-count"]', li);
    const comments = commentsNode ? toInt((norm(commentsNode.textContent).match(/\d+/) || ['0'])[0]) : 0;
    const likes = likesNode ? toInt((norm(likesNode.textContent).match(/-?\d+/) || ['0'])[0]) : 0;
  ...
  async function fetchTopicMedia(tid) {
    ...
    const promise = (async () => {
      if (window.HAA9Plugin && typeof window.HAA9Plugin.fetchTopicMedia === 'function') {
        try {
          const media = await window.HAA9Plugin.fetchTopicMedia(key);
          if (media && typeof media === 'object') {
            const pid = media.pid || media.mainPid || '';
            if (pid) state.pidCache.set(key, String(pid));
            state.topicCache.set(key, { expiresAt: Date.now() + CONFIG.topicCacheMs, value: media });
            writeTopicMediaLocalCache(key, media);
            return media;
          }
        } catch (error) {
          console.warn('HAA9 batch media failed, falling back:', error);
        }
      }
  ...
  function countryCodeFromProfile(profile = {}) {
    const raw = norm(profile.language_flag || profile.country || profile.nationality || profile.location || '');
    if (!raw) return '';
    ...
    const pairs = [
      ...
      ['philippines', 'PH'], ['菲律宾', 'PH'], ['japan', 'JP'], ['日本', 'JP'], ['korea', 'KR'], ['韩国', 'KR'],
      ['united states', 'US'], ['usa', 'US'], ['美国', 'US'], ['united kingdom', 'GB'], ['uk', 'GB'], ['英国', 'GB'],
      ['france', 'FR'], ['法国', 'FR'], ['germany', 'DE'], ['德国', 'DE'], ['india', 'IN'], ['印度', 'IN']
    ];
  ...
  async function fetchProfile(user) {
    ...
    const promise = (async () => {
      if (window.HAA9Plugin && typeof window.HAA9Plugin.fetchProfile === 'function') {
        try {
          const data = await window.HAA9Plugin.fetchProfile(user);
          ...
          console.warn('HAA9 batch profile failed, falling back:', error);
        }
      }
      ...
      return fetch(api, { credentials: 'same-origin', headers: { accept: 'application/json' } })
        ...
        .then(json => {
          const data = normalizeProfile(json);
          state.profileCache.set(key, data);
          safeJsonSet(profileCacheKey(user), { expiresAt: Date.now() + CONFIG.profileCacheMs, data });
          return data;
        })
  ...
  function decorateAvatar(li, profile) {
    ...
    if (!$('[component="user/status"]', host)) {
      const status = norm(profile.status || '').toLowerCase();
  ...
  function getTranslateSettings() {
    ...
    return {
      sourceLang: normalize(rawSource, 'auto'),
      targetLang: normalize(rawTarget, 'zh'),
      provider: saved.provider === 'ai' ? 'ai' : 'google',
      aiEndpoint: saved.aiEndpoint || '',
      aiModel: saved.aiModel || '',
      aiApiKey: saved.aiApiKey || '',
      aiPrompt: saved.aiPrompt || '你是专业论坛翻译助手。请把用户提供的内容从 {{sourceLang}} 翻译为 {{targetLang}}。保留原有语气、换行、链接、Markdown、代码块、用户名、表情和列表结构。只输出译文，不要解释。',
  ...
  async function translateText(text) {
    ...
    if (external) {
      ...
      if (settings.provider === 'ai') {
        ...
        const prompt = String(settings.aiPrompt || '')
          .replace(/{{\s*sourceLang\s*}}/gi, settings.sourceLang || 'auto')
          .replace(/{{\s*targetLang\s*}}/gi, settings.targetLang || 'zh');
        const res = await fetch(endpoint, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${settings.aiApiKey}` },
          body: JSON.stringify({ model: settings.aiModel, temperature: settings.temperature, messages: [{ role: 'system', content: prompt }, { role: 'user', content: clean }] })
        });
        if (!res.ok) throw new Error(`ai translate ${res.status}`);
        const data = await res.json();
        translated = normalizeTranslateOutput(data.output_text || (data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content) || '');
      } else {
  ...
  function createVideoCard(item, extraClass = '') {
    ...
    const handleMainVideoTap = event => {
      ...
      const isLowerExitZone = y > viewportH * 0.82;
  ...
  function createContent(media, options = {}) {
    ...
    if (!hasText && !hasImages && !hasAudios && !hasTiktoks) return null;

    const box = createElement('div', { class: `haa9-content ${options.extraClass || ''}`.trim() });
    ...
    if (hasImages) {
      const strip = createElement('div', { class: 'haa9-image-list haa9-bleed' });
      media.images.forEach(src => {
        const button = createElement('button', { class: 'haa9-image', type: 'button', dataset: { src } });
        button.appendChild(createElement('img', { src, alt: 'image', loading: 'lazy' }));
  ...
  function mediaSignature(media) {
    if (!media) return '';
    const images = (media.images || []).map(String);
    const audios = (media.audios || []).map(item => String((item && item.url) || item || ''));
    const tiktoks = (media.tiktoks || []).map(item => String((item && item.videoId) || item || ''));
    return [...images, ...audios, ...tiktoks].filter(Boolean).join('|');
  }

  function topicDetailPostContent(post) {
    if (!isElement(post)) return null;
    return $('[component="post/content"], .post-content, .content', post);
  }

  function scrubTikTokTextNodes(root) {
    if (!root || !document.createTreeWalker) return;
    const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
      acceptNode: node => {
        if (!node || !node.nodeValue || !node.nodeValue.trim()) return NodeFilter.FILTER_REJECT;
        if (node.parentElement && node.parentElement.closest('.haa9-detail-content, .haa9-video, .haa9-audio, script, style')) return NodeFilter.FILTER_REJECT;
        return (RE.tiktokGlobal.test(node.nodeValue) || RE.tiktokToken.test(node.nodeValue)) ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_REJECT;
      }
    });
    const nodes = [];
  ...
  function hideNativePostMedia(post, media) {
    const content = topicDetailPostContent(post);
    ...
    if (hasImages) {
      $$('img[src]', content).forEach(img => {
        img.classList.add('haa9-native-media-hidden');
        img.setAttribute('aria-hidden', 'true');
      });
    }

    if (hasAudios) {
      $$('audio, source[src]', content).forEach(node => {
        node.classList.add('haa9-native-media-hidden');
    ...
    $$('a[href]', content).forEach(link => {
      const href = link.getAttribute('href') || '';
      const isImage = hasImages && RE.imageExt.test(href);
      const isAudio = hasAudios && RE.audioExt.test(href);
      const isTikTok = hasTiktoks && (RE.tiktokOne.test(href) || RE.tiktokToken.test(href));
      if (!isImage && !isAudio && !isTikTok) return;
      link.classList.add('haa9-native-media-hidden');
      link.setAttribute('aria-hidden', 'true');
    });

    if (hasTiktoks) {
      $$('iframe, blockquote, section, article, div', content).forEach(node => {
        if (node.closest('.haa9-detail-content')) return;
        const html = String(node.outerHTML || '').toLowerCase();
        const cls = String(node.className || '').toLowerCase();
        const looksTikTok = html.includes('tiktok.com') || cls.includes('tiktok') || cls.includes('embed') || cls.includes('preview') || cls.includes('link-preview');
        if (!looksTikTok) return;
        node.classList.add('haa9-native-media-hidden');
        node.setAttribute('aria-hidden', 'true');
      });
      scrubTikTokTextNodes(content);
    }
  }

  function topicDetailPostItems(root = document) {
    if (!isTargetTopicPage()) return [];
    const posts = $$('li[component="post"], [component="post"][data-pid], [data-pid].post, .posts-list [data-pid]', root)
      .filter(item => isElement(item))
      .filter(item => !item.closest('#haa9-root, #haa9-tiktok-fullscreen'));
    return Array.from(new Set(posts));
  }

  function hydrateTopicDetailPost(post) {
    const content = topicDetailPostContent(post);
    if (!content) return;
    const media = parseMediaFromContent(content.innerHTML || '');
    const sig = mediaSignature(media);
    if (!sig) return;
    if (post.dataset.haa9DetailSig === sig && $('.haa9-detail-content', post)) return;

    $$('.haa9-detail-content', post).forEach(node => node.remove());
    hideNativePostMedia(post, media);

    const mediaOnly = Object.assign({}, media, { text: '' });
    const block = createContent(mediaOnly, { extraClass: 'haa9-detail-content', videoClass: 'haa9-detail-video' });
    if (!block) return;
    content.insertAdjacentElement('afterend', block);
    post.dataset.haa9DetailSig = sig;
    post.dataset.haa9DetailReady = '1';
    restoreTranslateButtons(block);
  }

  function hydrateTopicDetailPosts(root = document) {
    if (!isTargetTopicPage()) return;
    document.body.classList.add('haa9-mode', 'haa9-topic-detail-mode');
    topicDetailPostItems(root).forEach(post => hydrateTopicDetailPost(post));
  }
  ...
  async function decorateUserTopicUser(item) {
    if (!isElement(item) || $('.haa9-user-topic-user', item)) return;
    const user = userFromTopicItem(item);
    if (!user.userslug && !user.uid) return;
    const profile = await fetchProfile(user);
    if (!document.body.contains(item) || $('.haa9-user-topic-user', item)) return;
    const merged = Object.assign({}, user, {
      uid: String(profile.uid || profile.userId || profile.userid || user.uid || ''),
      username: user.username || profile.username || '',
      displayName: user.displayName || profile.displayname || profile.username || user.userslug || ''
    });
    const row = baseRow(item);
    const host = mainColumn(item);
    const header = makeUserHeader(merged, profile || {}, 'haa9-user-topic-user');
    if (host && host !== item) host.prepend(header);
    else if (row && row.parentNode === item) row.insertAdjacentElement('beforebegin', header);
  ...
  async function hydrateUserTopicItem(item) {
    ...
    try {
      const [media] = await Promise.all([fetchTopicMedia(tid), decorateUserTopicUser(item)]);
      ...
      $$('.haa9-user-topic-content, .haa9-user-topic-media, .haa9-user-topic-skeleton', item).forEach(node => node.remove());
  ...
  function cleanupComposerOnly() {
    closeComposer();
    $('#haa9-root')?.remove();
    if (typeof window.xRemoveCategoryToolbar === 'function') window.xRemoveCategoryToolbar();
  }

  function cleanupModeOnly() {
    document.body.classList.remove('haa9-mode', 'haa9-user-topics-mode', 'haa9-topic-detail-mode');
  }
  ...
  function ensureComposer() {
    if (!isCategoryRoute() || isTopicRoute()) return;
    if ($('#haa9-root')) return;
    ...
    root.innerHTML = html`
      ...
      <section id="haa9-composer" role="dialog" aria-modal="true">
        ...
        <div id="haa9-preview"><div id="haa9-image-preview" class="haa9-preview-card"><button type="button" id="haa9-remove-image" class="haa9-preview-remove">${icon('fa-xmark')}</button><img alt="preview"></div><div id="haa9-voice-preview" class="haa9-preview-card"><button type="button" id="haa9-remove-voice" class="haa9-preview-remove">${icon('fa-xmark')}</button><div class="haa9-voice-preview-inner"></div></div></div>
        ...
        <div class="haa9-compose-actions">
          ...
          <button type="button" class="haa9-tool" id="haa9-compose-translate" aria-label="翻译输入内容" title="翻译输入内容">${icon('fa-language')}</button>
        ...
        <input type="file" id="haa9-gallery-input" accept="image/*">
      </section>`;
    document.body.appendChild(root);
    bindComposer();
  }
  ...
  function bindComposer() {
    ...
    $('#haa9-remove-image').addEventListener('click', () => setPendingImage(null));
    $('#haa9-remove-voice').addEventListener('click', () => setPendingVoice(null, 0));
    $('#haa9-voice-btn').addEventListener('click', toggleRecording);
    $('#haa9-compose-translate')?.addEventListener('click', translateComposerText);
  ...
  async function translateComposerText(event) {
    ...
    const input = $('#haa9-text');
    const button = $('#haa9-compose-translate');
    const raw = input ? norm(input.value) : '';
    if (!input || !raw || (button && button.classList.contains('is-loading'))) return;
    if (button) {
      button.classList.add('is-loading');
      button.disabled = true;
    }
    try {
      const translator = typeof window.xTranslateText === 'function' ? window.xTranslateText : translateText;
      const out = await Promise.resolve(translator(raw));
      if (out) {
        input.value = String(out).trim();
        autoSizeComposer();
        input.focus();
      }
    } catch (error) {
      console.warn('HAA9 composer translate failed:', error);
      alertError(TEXT.translateFail);
    } finally {
      if (button) {
        button.classList.remove('is-loading');
  ...
  function setPendingImage(file) {
    if (state.composer.imageUrl) URL.revokeObjectURL(state.composer.imageUrl);
    state.composer.imageFile = file || null;
    state.composer.imageUrl = file ? URL.createObjectURL(file) : '';
    const card = $('#haa9-image-preview');
    if (!card) return;
    card.classList.toggle('is-show', !!file);
    const img = $('img', card);
    if (img) img.src = state.composer.imageUrl || '';
  }
  ...
  async function handleImageInput(event) {
    ...
    event.target.value = '';
    if (!file) return;
    if (!/^image\//i.test(file.type)) return alertError(TEXT.imageOnly);
  ...
  function createVoiceRecorder(stream, mimeType) {
    const options = { audioBitsPerSecond: CONFIG.voiceAudioBitsPerSecond || 16000 };
    if (mimeType) options.mimeType = mimeType;
    try {
      return new MediaRecorder(stream, options);
    } catch (error) {
      console.warn('HAA9 MediaRecorder bitrate options failed; retrying with compatible options:', error);
  ...
  async function startRecording() {
    ...
    try {
      ...
      state.composer.mediaRecorder = createVoiceRecorder(stream, mimeType);
  ...
  async function uploadToNodeBB(file) {
    ...
    form.append('cid', String(currentCid() || Number(CONFIG.cid) || ''));
  ...
  async function sendTopic() {
    ...
    if (!text && !state.composer.imageFile && !state.composer.voiceBlob) return alertError(TEXT.enterSomething);
    button.disabled = true;
    ...
    try {
      ...
      if (state.composer.imageFile) {
        $('#haa9-meta').textContent = TEXT.uploadImage;
        const url = await uploadToNodeBB(state.composer.imageFile);
        lines.push(`![image](${url})`);
      }
      await createTopic({ cid: currentCid() || Number(CONFIG.cid), title: buildTitle(text), content: lines.join('\n\n'), tags: [] });
  ...
  function bindLightbox() {
    ...
    const img = createElement('img', { alt: 'image' });
    box.appendChild(img);
    ...
    document.addEventListener('click', event => {
      ...
      if (!button) return;
      open(button.dataset.src || ($('img', button) && $('img', button).src) || '', event);
    }, true);
    ['click', 'pointerup', 'touchend'].forEach(type => {
      box.addEventListener(type, close, true);
      img.addEventListener(type, close, true);
    });
    document.addEventListener('keydown', event => {
      if (event.key === 'Escape' && box.classList.contains('is-show')) close(event);
    });
  }
  ...
  function run() {
    const onCategory = isCategoryRoute();
    const onTopic = isTopicRoute();
    ...
    if (onCategory) {
      ensureComposer();
      if (typeof window.xEnsureCategoryToolbar === 'function') window.xEnsureCategoryToolbar({ openComposer, translateScan: true });
    }
    ...
    } else if (isTargetTopicPage()) {
      bindLightbox();
      ensureTikTokResourceHints();
      ensureTikTokFullscreenRoot();
      hydrateTopicDetailPosts(document);
    } else if (onCategory) {
      cleanupModeOnly();
    ...
    if (typeof window.xBindNativeTitleTranslation === 'function') window.xBindNativeTitleTranslation(document);
    observeMutations();
  }

  function boot() {
    window.clearTimeout(state.bootTimer);
    state.bootTimer = window.setTimeout(run, 80);
  }
